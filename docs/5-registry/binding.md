...
这个为已知对象分配 ID 的过程称为对象绑定。一旦客户端像这样绑定到注册表，服务器就会多次发起全局事件，以告知它支持的接口。每个全局对象都有一个独一无二的名称，这个名称是一个无符号整数。接口字符串映射到协议中找到的接口名称：在上面 XML 中的 `wl_display` 就是这样一个名称的示例。此外，版本号也在这里定义——关于接口版本的更多信息，请查看附录 C。
